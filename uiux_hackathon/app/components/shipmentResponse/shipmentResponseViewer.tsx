...
export default function ShipmentResponseViewer({ data }: JsonResponseViewerProps) {
  ...
  return (
    <div className="container mx-auto p-4">
     ...
     <Button
        className="bg-secondary hover:bg-pink-600 text-white text-sm lg:text-base py-3 px-6 rounded-none shadow-md transition-all"
        ...
        <AlertDialogContent>
          ...
          <div className="grid gap-4 mt-4">
            <div className="flex items-center space-x-2">
              ...
              <span>{data.shipment_id || 'N/A'}</span>
            </div>

            <div className="flex items-center space-x-2">
              <Truck className="text-green-500" />
              <span className="font-semibold">Status:</span>
              <span className="capitalize">{data.status || 'N/A'}</span>
            </div>

            <div className="flex items-center space-x-2">
              <DollarSign className="text-yellow-500" />
              <span className="font-semibold">Cost:</span>
              <span>{`${data.shipment_cost?.amount || 0} ${data.shipment_cost?.currency || 'USD'}`}</span>
            </div>
          ...
          <Button
            ...
            {data.tracking_url && (
              <Link href={data.tracking_url}>
                <Button className="mt-4 bg-secondary hover:bg-pink-600 text-white">
                  Track Order
                </Button>
              </Link>
            )}
